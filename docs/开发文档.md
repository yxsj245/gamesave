...
| `IconPath` | `string?` | 游戏自定义图标路径（可选，优先于自动提取图标） |
...
**界面实现约束：**
- 主页的存档详情面板 `DetailsOverlay` 必须保持在列表视图与平铺视图之上
- 首页切换展示模式时，`GamesList` / `GamesTileView` 的 `Canvas.ZIndex` 只允许在动画期间临时提升；动画结束后必须恢复默认层级，避免把 `DetailsOverlay` 压到后面
...
| 4 | 自定义图标 | ❌ | 可选，选择图片作为游戏列表显示图标 |
| 5 | 云端服务商 | ❌ | 可选，选择已配置的云端服务商 |
...
>
> 自定义图标会优先显示在游戏卡片中；导出/导入游戏数据时，图标文件会和存档一起随游戏目录处理。
