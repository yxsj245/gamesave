...
### 🔎 存档目录自动探测
- 未设置存档目录时，启动游戏即进入探测模式
- 基于 **ETW 内核事件追踪**，实时监控游戏文件写入行为
- 探测结果按 **用户目录 / 游戏目录 / 其他目录** 三区块独立展示和评分
- 目录名含 `save`、`savegame` 等关键字时自动将得分提升至 99
- 勾选确认后一键写入存档路径，自动替换为环境变量
...
添加游戏时如果 **不填写存档目录**，点击「启动游戏」后会自动进入**存档探测模式**：

1. 应用会根据需要请求管理员权限（ETW 内核追踪所需）
2. 游戏正常启动，主窗口缩入系统托盘
3. 屏幕上弹出置顶的**探测结果窗口**，实时展示三个区块的疑似存档目录：
   - 📁 **用户目录**（AppData / Documents / Saved Games 等）
   - 🎮 **游戏安装目录**（exe 所在目录及子目录）
   - 📂 **其他目录**
4. 在游戏中执行一次存档操作后，探测窗口会立即显示捕获结果
5. 勾选正确的目录，点击**「确认选择」**即可自动写入存档路径

> 详细说明请参阅 [存档自动探测文档](docs/存档自动探测.md)
...
| [存档自动探测](docs/存档自动探测.md) | ETW 内核监控自动识别存档目录 |
